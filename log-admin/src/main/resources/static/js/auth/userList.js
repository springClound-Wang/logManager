    ...
    layui.use('table', function(){
        ...
        tableIns=table.render({
            ...
            ,cols: [[
                ...
                ,{field:'isJob', title:'是否在职',width:95,templet:'#jobTpl'}
...
function setJobUser(obj,id,nameVersion,checked){
    ...
    layer.confirm('您确定要把用户：'+name+'设置为'+userIsJob+'状态吗？', {
        ...
        /*if(status==="false"){
        	// false 是在职
    		$("#"+id+name).prop("checked",true).next().attr("class","layui-unselect layui-form-switch layui-form-onswitch").find("em").text("在职");
    	}else{
    		$("#"+id+name).prop("checked",false).next().attr("class","layui-unselect layui-form-switch").find("em").text("离职");
    	}*/
        
    });
    //form.render('checkbox');  // 刷新元素渲染
}
//提交表单
function formSubmit(obj){
	var currentUser=$("#currentOpt").html();
    if(checkRole()){
