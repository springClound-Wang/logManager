...
function parseList(res,currPage,index){
	 ...
	 if(null != res.list || res.list==''){
		 $.each(res.list, function (i, o) {
			  ...
			  content += "<td>" +
			  			 "<a class='layui-btn layui-btn-xs' onclick='selecteDetail("+o.id+","+currPage+",1);'>查看</a>"+
			  			 "<a class='layui-btn layui-btn layui-btn-warm layui-btn-xs' onclick='downExcel("+o.uuid+",\""+o.shopName+"\");'>下载</a>"+
			  			 "</td>";
			 });
...
//excel下载
var downloadPath = "//static.liyida.com";//查询路径
function downExcel(uuid,shopName){
	/* console.log(uuid+"_"+shopName); */
//	window.open(downloadPath+"/20160817131838_大VinVin.xlsx");
	window.open(downloadPath+"/"+uuid+"_"+shopName+".xlsx");
}