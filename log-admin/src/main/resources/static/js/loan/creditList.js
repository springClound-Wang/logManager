     ...
     $(function() {
        ...
        layui.use(['layer', 'form','laydate','table','util'], function(){
          ...
          //监听清空
      	 ...
      	 //监听返回
      	  form.on('submit(backSubmit)', function(data){
      		location.href="/infor/toCreditList?curr="+pageCurr;
      		  return false;
      	  });
      	 //监听刷新
          ...
          table.on('tool(creditTable)', function(obj){
              ...
              if(obj.event === 'see'){
            	  location.href="/infor/toCreditDetailPage?orderid="+data.orderID+"&userid="+data.userID+"&callback="+pathname+"&curr="+pageCurr;
...
//查询地址信息
function searchAddressInfo(id,type,work){
	//宜信 work是工作还是家庭
		$.post("/infor/getProCityArea",{"type":type,"id":id},function(data){
			if(data.msg=="success"){
				if(type==1&&work==1){
					$("#iaddr_prov").append("<option value='1'>"+data.data.name+"</option>");
				}else if(type==2&&work==1){
					$("#iaddr_city").append("<option value='2'>"+data.data.name+"</option>");
				}else if(type==3&&work==1){
					$("#iaddr_county").append("<option value='3'>"+data.data.name+"</option>");
				}
				
				if(type==1&&work==2){
					$("#work_prov").append("<option value='1'>"+data.data.name+"</option>");
				}else if(type==2&&work==2){
					$("#work_city").append("<option value='2'>"+data.data.name+"</option>");
				}else if(type==3&&work==2){
					$("#work_county").append("<option value='3'>"+data.data.name+"</option>");
				}
			}
			//重新渲染下form表单 否则下拉框无效
...
//渲染下拉框
function rendering(str,data,realid){
	$("#"+str).append(data);
	var ac = $("#"+str)[0];
	for (var i = 0; i < ac.length; i++) {
		if (ac[i].value == realid) {
				var text=ac[i].text;
			  //为下拉框选中值
			 $("#"+str).html("");
			 $("#"+str).append("<option value='1'>"+ text+"</option>");
			 //重新渲染下form表单 否则下拉框无效
			 layui.use(['form'], function(){
				var form = layui.form ;
				 form.render('select');
...
function examineFormSub(){
	...
	if(flag){
		$.post("/infor/examineCreditInfo",$("#examineForm").serialize(),function(data){
			if(data=="success"){
...
function bindingInfoUser(){
	...
	if(flag){
		$.post("/infor/bindingUserId",$("#bindingUserForm").serialize(),function(data){
				if(data == "success"){
