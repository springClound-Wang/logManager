...
import com.wupao.log.entity.RoleVO;
...
/**
 * @项目名称：wyait-manage
 * @包名：com.lyd.channel.controller.auth
 * @类描述：
 * @创建人：wyait
...
public class AuthController {
	...
	public String addRole(@RequestParam("permIds") String permIds, Role role) {
		...
		try {
			if(StringUtils.isEmpty(permIds)){
				logger.debug("添加角色并授权【lyd-admin.AuthCnotroller.addRole】===未选择权限！请求参数：角色数据role："+role+"，权限数据permIds："+permIds);
				return "未授权，请您给该角色授权";
			}
			if(null == role){
				logger.debug("添加角色并授权【lyd-admin.AuthCnotroller.addRole】===请填写完整的角色数据！请求参数：角色数据role："+role+"，权限数据permIds："+permIds);
				return "请您填写完整的角色数据";
			}
	...
	public String setRole(@RequestParam("rolePermIds") String permIds, Role role) {
		...
		try {
			if(StringUtils.isEmpty(permIds)){
				logger.debug("更新角色并授权【lyd-admin.AuthController.setRole】===未选择权限！请求参数：角色数据role："+role+"，权限数据permIds："+permIds);
				return "未授权，请您给该角色授权";
			}
			if(null == role){
				logger.debug("更新角色并授权【lyd-admin.AuthController.setRole】===请填写完整的角色信息！请求参数：角色数据role："+role+"，权限数据permIds："+permIds);
				return "请您填写完整的角色数据";
			}
